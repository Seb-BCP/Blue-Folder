...
function setupLiveUpdates() {
    ...
    document.getElementById('timesheet-body').addEventListener('input', (e) => {
        if (e.target.classList.contains('worker-input') || 
            e.target.classList.contains('client-input') ||
            e.target.classList.contains('site-input')) {
            updatePaySheets();
...
class BlueFolder {
    ...
    updateCharges() {
        ...
        timesheetRows.forEach(row => {
            ...
            const clientValue = row.querySelector('.client-input').value;
            const siteValue = row.querySelector('.site-input').value;
            const worker = row.querySelector('.worker-input').value;
            const classification = row.querySelector('.classification-select').value;
            
            if (!clientValue || !siteValue || !worker || !classification) return;
            
            const key = `${clientValue}-${siteValue}-${worker}`;
        ...
        clientData.forEach(data => {
            ...
            let daysWorked = Array.from(document.querySelectorAll(`#timesheet-body tr`))
                .filter(row => 
                    row.querySelector('.client-input').value === data.client &&
                    row.querySelector('.site-input').value === data.site &&
...
function updateReports() {
    ...
    chargesRows.forEach(row => {
        ...
        const totalCharge = parseFloat(cells[16].textContent.replace('$','')) || 0;
    ...
    chargesRows.forEach(row => {
        const cells = row.querySelectorAll('td');
        if (cells.length < 17) return;
        const charge = parseFloat(cells[16].textContent.replace('$','')) || 0;
        const classificationName = cells[3].textContent.trim();
        const classificationData = currentRates.get(classificationName);
        if (classificationData && classificationData.industry === 'Construction') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addRowButton) {
        addRowButton.addEventListener('click', () => {
            ...
            row.innerHTML = `
                <td><input type="text" class="client-input"></td>
                <td><input type="text" class="site-input"></td>
                <td><input type="text" class="worker-input"></td>
                <td>
                    <select class="classification-select">
                        <option value="">Select Classification</option>
